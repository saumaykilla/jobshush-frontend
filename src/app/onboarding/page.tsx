...
const Onboarding = () => {
  ...
  const steps = [
    { id: "method", title: "Choose Method", icon: Upload },
    { id: "personal", title: "Personal Info", icon: User },
    { id: "role", title: "Professional Profile", icon: Briefcase },
    { id: "education", title: "Education", icon: GraduationCap },
    { id: "experience", title: "Work Experience", icon: Building },
    { id: "skills", title: "Skills & Sections", icon: Star },
  ];
  ...
  const getNextSectionNumber = () => {
    const customSections = methods?.watch("customSections") || [];
  ...
  const baseDragHandler = DragReorderHandler(fields, move);

  const handleDragDrop = async (e: DragEndEvent) => {
    await baseDragHandler(e);
          ...
          <div className="space-y-6">
            <div className="text-center space-y-4">
              ...
              <p className="text-muted-foreground">
                Choose the method that works best for you. You can always edit
            ...
            <div className="grid gap-4">
              <Card
                className={`${isProcessing ? "opacity-50 cursor-not-allowed" : "cursor-pointer transition-all hover:border-primary"} ${
                  uploadMethod === "upload" ? "border-primary bg-primary/5" : ""
                }`}
                onClick={() => !isProcessing && setUploadMethod("upload")}
              >
              ...
              <Card
                className={`${isProcessing ? "opacity-50 cursor-not-allowed" : "cursor-pointer transition-all hover:border-primary"} ${
                  uploadMethod === "manual" ? "border-primary bg-primary/5" : ""
                }`}
                onClick={() => !isProcessing && setUploadMethod("manual")}
              >
                <CardContent className="p-6">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-lg bg-primary/10 flex items-center justify-center">
                    ...
                    <div className="flex-1">
                      <h3>Fill Out Manually</h3>
                      <p className="text-muted-foreground">
                        Enter your information step by step using our guided
                        form
                      </p>
                    </div>
                  </div>
        ...
        return (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-muted-foreground">
                Add your professional summary and links
              </p>
            </div>
            <RoleDetails />
          </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div className="space-y-2">
                <h2>Skills & Additional Sections</h2>
                <p className="text-muted-foreground">
                  Add your skills and any custom sections you&apos;d like to
                  include
                </p>
              </div>
              <Button
                ...
                >
                  {fields?.map((lineItem, idx) => {
                    ...
                    const currentSectionName =
                      methods?.watch(`customSections.${idx}.sectionName`) ||
                      sectionNames[idx];
                    const isDuplicate =
                      sectionNames.filter(
                    ...
                    return (
                      <div key={lineItem?.id} className="space-y-4">
  ...
  const handleNext = async () => {
    ...
    switch (currentStep) {
      ...
      case 2: {
        isValid = await trigger(["roleDetails"]);
        break;
      }
      case 3: {
      ...
      case 4: {
        isValid = await trigger(["workExperience"]);
        break;
      }
      case 5: {
        isValid = await trigger(["skills", "customSections"]);
        if (isValid) {
          // Ensure sectionOrder is up to date before submission
          setIsProcessing(true);
          updateSectionOrder();
          const payload = {
            personalDetails: methods?.watch()?.personalDetails,
            roleDetails: {
              ...methods?.watch()?.roleDetails,
              linkedInURL:
                methods?.watch()?.roleDetails?.linkedInURL || undefined,
            },
            education: {
              fieldName: methods?.watch()?.education?.fieldName,
              lineItem:
                methods?.watch()?.education?.lineItem?.map((items) => ({
                  ...items,
            ...
            skills: {
              ...methods?.watch()?.skills,
              fieldName: "Skills",
            },
            workExperience: {
              fieldName: "Work Experience",
              lineItem:
                methods?.watch()?.workExperience?.lineItem?.map((items) => ({
                  ...items,
                  startDate: items?.startDate
                    ? new Date(items.startDate).toISOString()
                    : "",
            ...
            sectionOrder: methods?.watch()?.sectionOrder,
            customSections: methods?.watch()?.customSections,
            template: methods?.watch()?.template as "Classic" | "Modern",
          };
          try {
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h1>Profile Setup</h1>
            <p className="text-muted-foreground">
              Complete your profile to get started with Excelify
            </p>
          </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <Button
          onClick={handleNext}
          disabled={
            uploadMethod === null || uploadMethod === "upload" || isProcessing
          }
        >
